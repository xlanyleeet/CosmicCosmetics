...
public class ProjectileCosmetics extends InventoryGUI {
    ...
    protected void createInventory() {
        this.inventory = Bukkit.createInventory(null, 27, ChatColor.translateAlternateColorCodes('&', lang.getProjectileEffectsMenuName()));
    }
    ...
    public void decorate(Player player) {
        ...
        activeEffect = getActiveEffect(player, EffectForm.PROJECTILE);

        int slot = 9;
        for (EffectType effectType : getFormEffects(EffectForm.PROJECTILE)) {
            ...
            if (checkPlayerPermission(player, "cosmetics.projectile."+effectType.name().toLowerCase())) {
        ...
        return new InventoryButton()
                ...
                .consumer(event -> {
                    if (activeEffect == effectType) {
                        removeActiveEffect(player, EffectForm.PROJECTILE);
                        sendMessage(player, lang.getEffectDisabled().replace("%effect_name%", getEffectDisplayName(effectType)), true);
                    } else {
                        setActiveEffect(player, EffectForm.PROJECTILE, effectType);
