...
public class TrailCosmetics extends InventoryGUI {
    ...
    protected void createInventory() {
        this.inventory = Bukkit.createInventory(null, 27, ChatColor.translateAlternateColorCodes('&', lang.getTrailEffectsMenuName()));
    }
    ...
    public void decorate(Player player) {
        ...
        activeEffect = getActiveEffect(player, EffectForm.TRAIL);

        int slot = 10;
        for (EffectType effectType : getFormEffects(EffectForm.TRAIL)) {
            ...
            if (checkPlayerPermission(player, "cosmetics.trail."+effectType.name().toLowerCase())) {
